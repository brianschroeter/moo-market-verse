DROP FUNCTION IF EXISTS public.get_channel_membership_breakdown();
...
BEGIN
    ...
    ranked_channels AS (
        SELECT
            ...
            ((clc.crown_count * 500) + (clc.paypig_count * 100) + (clc.cash_cow_count * 5))::NUMERIC AS total_usd_value,
            RANK() OVER (ORDER BY ((clc.crown_count * 500) + (clc.paypig_count * 100) + (clc.cash_cow_count * 5)) DESC, clc.channel_name ASC) AS calculated_rank
        FROM
